...
function conectaDb() {
    try {
        ...
        print "      <p>Error: No puede conectarse con la base de datos. <br/>Revise si están habilitados <strong>Apache</strong> y <strong>MySQL</strong></p>\n";
        print "\n";
...
function guardarCambioTurno($texto, $valor) {
    try {
        global $dbHac;
        $db = conectaDb();
        $conn = new PDO("mysql:host=localhost;dbname=$dbHac", MYSQL_USUARIO, MYSQL_PASSWORD);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $registro1 = "INSERT INTO $dbHac.cambio_turno (texto,tipo) VALUES ('$texto','$valor')";
        if ($conn->query($registro1)) {
            unset($texto);
            header("Location:../../index.php");
            die();
            exit();
...
function obtener_bitacora() {
    try {
        ...
        $conn = new PDO("mysql:host=localhost;dbname=$dbHac", MYSQL_USUARIO, MYSQL_PASSWORD);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $consulta = "SELECT COUNT(*) FROM $dbHac.bitacora";
        $conn = $db->query($consulta);
        if (!$conn) {
            print "<p>Error en la consulta.</p>\n";
            ...
            $consulta = "SELECT * FROM $dbHac.bitacora LIMIT 100";
            $resultado = $db->query($consulta);
            return $resultado;
        }
    } catch (PDOException $e) {
...
function borraRegistro($nombre_tabla, $id, $valores) {
    try {
        global $dbHac;
        $db = conectaDb();
        $conn = new PDO("mysql:host=localhost;dbname=$dbHac", MYSQL_USUARIO, MYSQL_PASSWORD);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $consulta = ("DELETE FROM $dbHac.$nombre_tabla WHERE $id IN ($valores)");
        if ($conn->exec($consulta)) {
            echo "Se ha eliminado el registro correctamente";
            unset($nombre_tabla, $id, $valores);
            header("Location:../../index.php?o=Se han borrado correctamente los datos");
            die();
            exit();
        } else {
            echo "Error";
            //$_SESSION["txt_alerta"] = TRUE;
            header("Location:index.php?error_bd=Error. No se ha borrado ningún registro");
            die();
            exit();
        }
    } catch (PDOException $e) {
...
/*function borraCt($valores) {
    try {
        global $dbHac;
        $db = conectaDb();
        $conn = new PDO("mysql:host=localhost;dbname=$dbHac", MYSQL_USUARIO, MYSQL_PASSWORD);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $consulta = $conn->query("DELETE FROM $db.cambio_trurno WHERE id_c_t IN ($valores)");
        //$q = $this->pdo->prepare($consulta);
        if ($consulta) {
            header("Location:index.php");
            die();
            exit();
